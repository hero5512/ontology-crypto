...
func BenchmarkAggregateSign(b *testing.B) {
	...
	if !flag {
		b.Error("party2EphemeralKey test fail")
	}
	...
	if !flag {
		b.Error("party1EphemeralKey test fail")
	}
	...
	if keyAgg1.APK.X.Cmp(keyAgg2.APK.X) != 0 {
		b.Error("agg1 not equal to agg2")
	}
	...
	if !(party1RTag.X.Cmp(party2RTag.X) == 0) {
		b.Error("parth1RTag not equal to party2RTag")
	}
	...
	if !(party1H0.Cmp(party2H0) == 0) {
		b.Error("party1H0 not equal to party2H0")
	}
	Sign(party1EphemeralKey, party1H0, *party1PrivateKey.(*ec.PrivateKey).PrivateKey, keyAgg1.Hash)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		Sign(party2EphemeralKey, party2H0, *party2PrivateKey.(*ec.PrivateKey).PrivateKey, keyAgg2.Hash)
	}
}

func BenchmarkAggregateVeri(b *testing.B) {
	isMuSig := true
	message := []byte{79, 77, 69, 82}
	party1PrivateKey, party1PublicKey, err := keypair.GenerateKeyPair(keypair.PK_AGGSIGN, keypair.SECP256K1)
	if err != nil {
